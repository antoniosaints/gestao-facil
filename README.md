...
- O cardápio público usa `/api/v1/restaurante/publico/:slug/cardapio` e entrega também a referência da logo configurada na conta. A criação exige `Idempotency-Key`, recalcula preços, adicionais, pedido mínimo e frete no servidor e devolve um token opaco de acompanhamento.
