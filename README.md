...
- O fluxo recorrente agora cobre exclusão controlada de planos e assinaturas, geração de cobrança no gateway por ciclo, cancelamento e estorno para PIX/boleto e reajuste com cancelamento da cobrança pendente anterior + recriação automática da nova cobrança.
...
- O worker `cronJobsWorker.ts` também passa a processar recorrência de assinaturas, gerando ciclos vencidos e tentando acionar automações financeiras do módulo quando a assinatura estiver configurada para isso, inclusive para link de pagamento quando o gateway suportar o fluxo.
