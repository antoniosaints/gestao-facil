...
- Os templates de lembrete de inadimplência aceitam os placeholders `{cliente}`, `{descricao}`, `{valor}` (total pendente), `{valorParcela}`, `{vencimento}`, `{parcela}` e `{totalParcelas}`; a UI deve oferecer inserção por clique e o backend mantém placeholders desconhecidos intactos.
