...
- `ParametrosConta.estiloUi` é uma preferência de navegação por conta, aceita apenas `PADRAO` e `CARDS` no contrato de parâmetros e é distribuída junto da atualização de sessão para que todos os usuários da conta recebam a mesma experiência.
...
- Webhooks de WhatsApp devem ser idempotentes por instância/evento, descartar grupos, canais, status/stories e presença antes da persistência, e guardar em `WhatsAppWebhookEvento` apenas o envelope reduzido necessário para processar mensagens 1:1, mídia, citações, reações, revogações, delivery/status e conexão. Eventos concluídos têm retenção máxima de dois dias; pendentes/em processamento são preservados para não perder mensagens. Emita Socket.IO apenas para `conta:<contaId>` e nunca dependa exclusivamente de polling para atualizar a inbox.
- A correlação de delivery/status usa o `messageId` devolvido pela W-API como `WhatsAppMensagem.externalMessageId`. IDs locais (`erp-*`/`agent-*`) servem somente durante o estado pendente; o processador pode reconciliar dados legados pelo `rawPayload` e deve encerrar imediatamente como ignorado, sem retry, um delivery que pertença a uma saída não rastreada pelo atendimento.
- O Socket.IO roda com **Redis adapter** e `transports: ["websocket"]` (`app/utils/socket.ts`) porque o app principal sobe em cluster PM2 (`instances: max`): o adapter garante que qualquer `emit` (inclusive os disparados no handler de webhook) alcance clientes conectados em qualquer worker, e o transporte WebSocket puro dispensa sticky session. Ao adicionar novos eventos de realtime, continue emitindo por `getIO()` no processo do server — não presuma processo único.
- Antes de remover algo de `views` ou `public`, confirmar se a rota ou fluxo legado ainda está em uso.
- O handshake Socket.IO valida o JWT e grava `userId`, `contaId` e elegibilidade de presença em `socket.data`; entradas e saídas de sala só são aceitas para a conta assinada no token. `services/administracao/assinanteActivityService.ts` consulta todas as instâncias pelo Redis adapter, deduplica usuários com várias abas e exclui impersonações da atividade exibida no modo CEO.
ainda está em uso.
