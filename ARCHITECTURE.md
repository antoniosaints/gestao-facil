...
- A correlação de delivery/status usa o `messageId` devolvido pela W-API como `WhatsAppMensagem.externalMessageId`. IDs locais (`erp-*`/`agent-*`) servem somente durante o estado pendente; o processador pode reconciliar dados legados pelo `rawPayload` e deve encerrar como ignorado, após três tentativas, um delivery que pertença a uma saída não rastreada pelo atendimento.
