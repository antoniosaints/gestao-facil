...
- No domínio `restaurante`, respostas seguem `{ data, meta?, requestId }` e erros seguem `{ error: { code, message, details?, requestId } }`. O controller administra itens, grupos e zonas sempre pelo `contaId` autenticado; a prévia e a criação pública compartilham o recálculo de catálogo, seleções, cobertura, pedido mínimo e frete, e a criação persiste snapshots dentro da transação.
