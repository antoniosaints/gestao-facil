...
function geranetPayload(invoice: any, auth: Awaited<ReturnType<typeof getGeranetCredentials>>) {
  ...
  return { acao: "emitir", modeloDocumento: "nfe", nomeSistema: "Gestão Fácil", certificadoDigital: auth.hex, senhaCertificadoDigital: auth.password, ambiente: invoice.ambiente === "PRODUCAO" ? "1" : "2", modelo: invoice.modelo, ufEmitente: address.uf, ...(invoice.tipo === "NFCE" ? { idCsc: auth.csc?.id, csc: auth.csc?.token } : {}), nfe: {
    ...
    itens: invoice.Itens.map((item: any) => { const tax = item.tributacaoJson || {}; return { desconto: "0.00", frete: "0.00", seguro: "0.00", outro: "0.00", quantidade: decimal(item.quantidade, 4), valorUnitario: decimal(item.valorUnitario, 4), valorTotal: decimal(item.valorTotal), ncmProduto: item.ncm, cest: item.cest || "", tipoItem: tax.tipoItem || "00", eanProduto: tax.ean || "SEM GTIN", codigoProduto: String(item.produtoId), nomeProduto: item.descricao, cfop: item.cfop, unidadeMedidaProduto: item.unidade || "UN", origemProduto: String(item.origem), ...(isSimple ? { icmsCsosn: tax.icmsCsosn } : { icmsCst: tax.icmsCst, icmsAliquota: decimal(tax.aliquotaIcms) }), pisCst: tax.pisCst || "08", pisAliquota: decimal(tax.aliquotaPis), cofinsCst: tax.cofinsCst || "08", cofinsAliquota: decimal(tax.aliquotaCofins) }; }),
  } };
}
