...
- `services/restaurante/pricing.ts` concentra sabores e frete fixo; `deliveryZone.ts` resolve cobertura; `payment.ts` integra Pix/Checkout Pro; `catalogQuery.ts` mantém o contrato de grupos com opções ativas usado pelo catálogo privado. `production.ts` despacha itens por categoria, pode exigir destino ativo para todos os itens internos e deriva o estado global apenas dos pontos obrigatórios. `printing.ts` renderiza o ticket térmico, enfileira com deduplicação e controla lease, retentativa e fallback. `catalogPolicy.ts` valida os grupos.
