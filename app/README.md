...
- Webhooks do WhatsApp entram por `POST /api/whatsapp/webhooks/:instanceId`, sem JWT; a W-API identifica a instância pelo caminho e não repassa query/header de segredo. Antes de persistir, o endpoint descarta grupos, canais, status/stories e presença; para eventos 1:1 e de status/conexão necessários, armazena somente o payload reduzido usado pelo Atendimento. O worker MySQL processa contato, conversa e mensagem em transação e remove eventos concluídos após dois dias.
- Mensagens de saída persistidas pelo atendimento trocam o ID temporário pelo `messageId` retornado pela W-API. O worker usa esse identificador para aplicar delivery/status, reconcilia payloads antigos e encerra imediatamente deliveries não rastreados como ignorados, sem retry.
...
- A administração de assinantes expõe `POST /api/admin/assinantes/:id/enviar-recuperacao-senha-root` somente ao superadmin fora de impersonação. A emissão do token e o enfileiramento do e-mail devem reutilizar `services/auth/passwordRecoveryService.ts`, compartilhado com a recuperação pública.
...
- A listagem `GET /api/admin/assinantes` também entrega presença e último login por conta. A presença vem de `services/administracao/assinanteActivityService.ts`, usa sockets autenticados e não considera sessões de suporte; `controllers/auth/login.ts` persiste `Usuarios.ultimoLoginEm` somente após login por senha bem-sucedido.
